...
def load_chain_data(ticker: str) -> dict:
    ...
    logger.debug(f"Loading chain data for {ticker}")
    data = fetch_and_save_chain(ticker)
    logger.debug("Successfully loaded data")
    logger.debug(f"Call expirations: {len(data.get('callExpDateMap', {}))}")
    logger.debug(f"Put expirations: {len(data.get('putExpDateMap', {}))}")
    return data
...
def _bs_unit_gamma(S, K, vol, T, r=0.0, q=0.0):
    ...
    logger.debug("Black-Scholes gamma calculation:")
    logger.debug(f"  S={S}, K={K}, vol={vol}, T={T}")
    logger.debug(f"  d1={d1}, gamma={gamma}")
    return gamma

def _dollar_gamma(unit_gamma, S, OI):
    """Convert unit gamma to dollar gamma value."""
    result = unit_gamma * OI * 100 * S * S * 0.01
    logger.debug("Dollar gamma calculation:")
    logger.debug(f"  unit_gamma={unit_gamma}, S={S}, OI={OI}")
...
def run_gex_analysis(chain_json: dict, ticker: str, spot: float = None, selected_expiry: str = None):
    ...
    """
    logger.debug("Starting GEX Analysis")
    logger.debug("Data source structure:")
    logger.debug(f"Keys in chain_json: {list(chain_json.keys())}")
    ...
    logger.debug("Expiration Analysis")
    logger.debug(f"All available expirations: {len(all_expirations)}")
    logger.debug(f"Valid upcoming expirations: {valid_exps}")
    ...
    spot = float(spot)
    logger.debug(f"Underlying price: {spot}")
    ...
    logger.debug("GEX Calculations")
    logger.debug(f"Total Call GEX: {g_agg['CallGEX'].sum() / 1e9:+.6f}Bn")
    logger.debug(f"Total Put GEX: {g_agg['PutGEX'].sum() / 1e9:+.6f}Bn")
    logger.debug(f"Net Total GEX: {g_agg['TotalGEX'].sum():+.6f}Bn")
    ...
    logger.debug("Profile Calculation")
    logger.debug(f"Number of unique strikes: {len(latest_exp_data)}")
    logger.debug(f"Strike range: {latest_exp_data.index.min():.2f} to {latest_exp_data.index.max():.2f}")
    
    levels = np.linspace(0.5 * spot, 1.5 * spot, 40)
    logger.debug(f"Price levels for analysis: {levels[0]:.2f} to {levels[-1]:.2f}")
    logger.debug("Starting Gamma Profile Calculation")
    profile = []
    logger.debug("First few rows of latest_exp_data:")
    logger.debug("\n%s", latest_exp_data.head())
    
    profile = []
    print("\nCalculating profile for each price level:")
    for i, lvl in enumerate(levels):
        unit_sum = 0
        if i < 2:  # Log detailed calculation for first two levels only
            logger.debug(f"Price level {lvl:.2f}:")
            
        for strike, row in latest_exp_data.iterrows():
            logger.debug(f"Processing strike {strike} at price level {lvl}:")
            logger.debug(f"Call data - IV: {row.CallIV}, DTE: {row.CallDTE}, OI: {row.CallOI}")
            logger.debug(f"Put data - IV: {row.PutIV}, DTE: {row.PutDTE}, OI: {row.PutOI}")
            ...
            logger.debug(f"Unit gammas - Call: {g_c}, Put: {g_p}")
            ...
            logger.debug(f"Dollar gamma - Call: {call_contribution}, Put: {put_contribution}")
            
            contribution = call_contribution + put_contribution
            logger.debug(f"Total contribution: {contribution}")
            
            unit_sum += contribution
            
            if i < 2 and abs(contribution) > 0.0001:
                logger.debug(f"  Strike {strike:.1f}: Call(gamma={g_c:.6f} * OI={row.CallOI}) + (-Put(gamma={g_p:.6f} * OI={row.PutOI})) = {contribution:.6f}")
          # Scale to billions here, once all contributions are summed
        gamma_dollar = unit_sum / 1e9
        profile.append(gamma_dollar)
        
        if i < 2:
            logger.debug(f"  Level {lvl:.2f} total: {gamma_dollar:.6f}")
    
    profile = np.array(profile)
    zero_gamma = None
    
    logger.debug("Zero Gamma Search")
    logger.debug(f"Profile shape: {profile.shape}")
    logger.debug("Profile values at each price level:")
    for lvl, prof in zip(levels, profile):
        logger.debug(f"Price: {lvl:.2f}, Gamma: {prof:.6f}")
    ...
    if len(significant_profile) > 0:
        logger.debug(f"Significant profile min/max: {significant_profile.min():.6f}/{significant_profile.max():.6f}")
    else:
        logger.debug("No significant gamma values found")
    ...
    logger.debug(f"Profile contains positive values: {has_positive}")
    logger.debug(f"Profile contains negative values: {has_negative}")
    
    # Check for sign changes
    sign_changes = np.where(np.diff(np.sign(profile)) != 0)[0]
    logger.debug(f"Number of sign changes found: {len(sign_changes)}")
    
    if sign_changes.size:
        i = sign_changes[0]
        zero_gamma = levels[i] - profile[i] * (levels[i + 1] - levels[i]) / (profile[i + 1] - profile[i])
        logger.debug(f"Zero gamma calculated at: {zero_gamma:.2f}")
    else:
        logger.debug("No zero gamma found - no sign changes in profile")
        
    # Verify the calculated zero gamma is reasonable
    if zero_gamma is not None:
        if not (0.5 * spot <= zero_gamma <= 1.5 * spot):
            logger.warning(f"Zero gamma {zero_gamma:.2f} outside expected range [{0.5 * spot:.2f}, {1.5 * spot:.2f}]")
            zero_gamma = None
